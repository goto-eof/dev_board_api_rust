...
use chrono::Utc;
use entity::{db_permission, db_role_permission, db_user_role};
use jsonwebtoken::{
        ...
        .and_then(
            move |authorization: Option<String>, permission_name: String| async move {
                ...
                return Err(warp::reject::custom(Unauthorized {
                    error_message: "You have not permission to access to this resource".to_string(),
                }));
            },
        );
}
...
pub fn generate_jwt(user_id: i32) -> Result<String, Error> {
    let expiration = Utc::now()
        .checked_add_signed(chrono::Duration::seconds(SETTINGS.jwt_ttl))
        .unwrap()
        .timestamp();
