...
pub async fn get_by_id(id: i32) -> Result<db_board::Model, DevBoardGenericError> {
...
pub async fn get_all() -> Result<Vec<db_board::Model>, DevBoardGenericError> {
    let db = DB_POOL.get().await;

    let result = db_board::Entity::find()
        .order_by_asc(db_board::Column::Id)
        .all(db)
        .await;
...
pub async fn create(json_data: serde_json::Value) -> Result<db_board::Model, DevBoardGenericError> {
    let db = DB_POOL.get().await;
